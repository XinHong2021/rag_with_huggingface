...
class Vectordatabase:
    ...
    def get_vector(self,EmbeddingModel, docs)->List[List[float]]:
        self.vectors = []
        for doc in tqdm(self.docs):
            # print(doc)
            self.vectors.append(EmbeddingModel.get_embedding(doc))

        # self.db = FAISS.from_documents(docs, EmbeddingModel.embedding)
        # faiss_index = self.db.index
        # index_map = self.db.index_to_docstore_id
        # self.vectors = []
